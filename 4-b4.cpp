...
int menu()
{
	cout << "1.用I、J、K、L键控制上下左右(大小写均可，边界停止)" << endl 
		 ...
		 <<"6.用I、J、K、L键控制上下左右(按左箭头键不向下移动，大小写均可，边界停止" << endl 
		 <<"0.退出" << endl 
		 << "[请选择0-6] ";
...
void move_by_ijkl(int u, int d, int l, int r, bool tag_huirao, bool five_six)
{
	//在循环中，若只检查ijkl的ASCII码，而不对224进行特殊处理
    //左方向键的224+75会在忽略224以后被当成K处理
	int input;
...
void move_by_arrow(int u, int d, int l, int r, bool tag_huirao)
{
	int input;
	const HANDLE hout = GetStdHandle(STD_OUTPUT_HANDLE);
	srand((unsigned int)(time(0)));
